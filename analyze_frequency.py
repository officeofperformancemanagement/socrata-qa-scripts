...
# write output csv
fieldnames = ['id', 'name', 'data_updated_at', 'updated_at', 'most_recent_found', 'frequency', 'recently_updated', 'median_days_between_entries', 'notes']
...
for base, asset in assets:
  ...
  if frequency is None or frequency.lower().strip() not in ['daily', 'weekly', 'monthly']:
  ...
  if temporal:
    ...
    with open(download_path) as f:
      ...
      median_days_between_entries = min(median_diffs) if len(median_diffs) > 0 else "n/a"
